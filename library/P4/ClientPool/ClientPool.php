...
class ClientPool extends ConnectedAbstract
{
    ...
    *                                               by default this is false and only local and stream depots are mapped
    ...
    /**
     * Clean up any stream spec edit that have been which prevent us from reusing this clients.
     */
    public function clearStreamSpec()
    {
        try {
            // Try and revert stream spec edits
            $this->getConnection()->run('stream', ['revert']);
        } catch (P4Exception $ex) {
            // As we can't confirm that we are cleaning a streams client going to catch
            // the error and ignore it.
            $client = $this->getConnection()->getClient();
            Logger::log(
                Logger::TRACE,
                "P4 (". spl_object_hash($this) .") Failed to revert Stream for workspace (" . $client . ") with error "
                . $ex->getMessage()
            );
        }
    }
