...
class Client extends PluralAbstract
{
    ...
    public static function exists($id, ConnectionInterface $connection = null)
    {
        ...
        $clients = static::fetchAll(
            array(
                ...
                static::FETCH_EDGE_CLIENTS => true,
            ),
            $connection
        );
    ...
    public function setView($view)
    {
        ...
        foreach ($view as $entry) {
            ...
            // We need to take account of the fact that each part of the entry could possibly contain
            // spaces so it is not sufficient to split on space. We do know however that paths with
            // spaces with be surrounded by quotes otherwise Perforce would not have accepted them.
            // Perforce does also not permit a space immediately after the first quote in any part
            // This explode copes with entries like
            //
            // abc def
            // "abc" def
            // "/a/b/and c" "def"
            // "/a/b/and c" "de f"
            $delimiter = strpos($entry, '" ') === false ? ' ' : '" ';
            $validate  = explode($delimiter, $entry);
            if (count($validate) != 2 || trim($validate[0]) === '' || trim($validate[1]) === '') {
                throw new \InvalidArgumentException(
                    "Each view entry must contain two paths, no more, no less."
                );
            }
            // The parsed view expects no quoting
            $parsedView[] = str_replace('""', '"', $entry);
        };
