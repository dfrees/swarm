...
class Change extends PluralAbstract implements ResolvableInterface
{
    ...
    // Change command field names
    const STREAM_HAVE_CHANGE = 'streamHaveChange';
     ...
     * @throws P4Exception                  for a failure such as unable to get a connection
     */
    ...
    public function setJobs($jobs)
    {
        ...
        foreach ($jobs as $key => $job) {
            ...
            // If the job id is not a string or numeric throw.
            if (!is_string($job) && !is_numeric($job)) {
                throw new \InvalidArgumentException('Each job must be a string or numeric.');
            }
        }
        ...
        if (!isset($this->cache[$cacheKey])) {
            ...
            foreach ($data as $key => $values) {
                ...
                if (strpos($key, 'job') === 0) {
                    ...
                    // This is a stream change
                    $stream['type']      = $key;
                    $stream['depotFile'] = $values;
                } elseif ($key === static::STREAM_HAVE_CHANGE) {
                    $stream['action'] = 'edit';
                    $stream['rev']    = $values;
                }
                if (is_array($values)) {
            ...
            $this->cache[$cacheKey] = array_merge($stream ? [$stream] : [], $files);
        }
    ...
    public function isRemoteEdgeShelf()
    {
        ...
        if (count($files)) {
            ...
            // If we cannot see the files try a describe without shelve (-S) to see if we
            // can see files that way. This caters for the a direct submit from an edge
            // client which we want to allow
            $files = $this->getFileData(false, false);
            if (!$this->cache['shelved'] || count($files)) {
                return false;
            }
        }
