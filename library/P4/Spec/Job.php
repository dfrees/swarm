...
class Job extends PluralAbstract
{
     ...
     * @param int $max  value to pass to the '-m' flag to limit results
    ...
    public function getChanges($max = 0)
    {
        ...
        $parameters = array('-j', $this->getId());
        // Use max if specified and it is a valid positive integer, otherwise ignore it
        if ((is_int($max) || ctype_digit($max)) && (int)$max > 0) {
            array_push($parameters, '-m', $max);
        }
        ...
        if (!isset($this->cache['changes']) || !is_array($this->cache['changes'])) {
            ...
            $data                   = $this->getConnection()->run('fixes', $parameters)->getData();
            foreach ($data as $fix) {
     ...
     * @throws \P4\Exception
     */
    public function getChangeObjects($max = 0)
    {
        // just skip to an empty iterator if we have no fixes
        if (!$this->getChanges($max)) {
            return new FieldedIterator;
        }
        ...
        if (!isset($this->cache['changeObjects'])
            ...
            $this->cache['changeObjects'] = Change::fetchAll(
                array(Change::FETCH_BY_IDS => $this->getChanges($max)),
                $this->getConnection()
            );
        }
