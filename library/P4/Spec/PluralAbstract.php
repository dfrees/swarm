...
abstract class PluralAbstract extends SingularAbstract
{
    ...
    // This was introduced to get stream client views. Won't work with classic. For classic you will need
    // to setup "p4 configure set server.global.client.views=1", which promotes classic client views to commit.
    const FETCH_EDGE_CLIENTS = 'EdgeClients';
     ...
     *                             FETCH_EDGE_CLIENTS - For stream client on edge server. Won't work with classic
     *                                                  clients.
     *                                                  Note: For classic clients enable server.global.client.views=1
     *
     * @param   ConnectionInterface     $connection optional - a specific connection to use.
    ...
    protected static function getFetchAllFlags($options)
    {
        ...
        if (isset($options[self::FETCH_EDGE_CLIENTS])) {
            $flags[] = "-a";
        }
