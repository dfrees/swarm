...
class Group extends PluralAbstract
{
    ...
    // Allows either FETCH_BY_NAME or FETCH_BY_IDS to search by name
    ...
    protected $fields = array(
        ...
        self::FIELD_SUBGROUPS => array(
            'accessor'  => 'getSubgroups',
            'mutator'   => 'setSubgroups'
        ),
        self::FIELD_OWNERS => array(
            'accessor'  => 'getOwners',
            'mutator'   => 'setOwners'
        ),
        self::FIELD_USERS => array(
            'accessor'  => 'getUsers',
            'mutator'   => 'setUsers'
        )
    );
    ...
    public static function fetchAll($options = array(), ConnectionInterface $connection = null)
    {
        ...
        $handler->setOutputCallback(
            function ($data, $type) use ($groups, &$group, &$count, $options, $max, $filter, $class, $connection) {
                ...
                if ($data['isSubGroup']) {
                    $group[self::FIELD_SUBGROUPS][] = $data['user'];
                }
                if ($data['isOwner']) {
                    $group[self::FIELD_OWNERS][] = $data['user'];
                }
                if ($data['isUser']) {
                    $group[self::FIELD_USERS][] = $data['user'];
                }
     ...
     * @param array|null        $params     params to pass to the delete
    ...
    {
        $entries = count($this->get(self::FIELD_SUBGROUPS))
                 + count($this->get(self::FIELD_OWNERS))
                 + count($this->get(self::FIELD_USERS));
    ...
    {
        $subGroups = $this->getRawValue(self::FIELD_SUBGROUPS) ?: array();
        sort($subGroups, SORT_STRING);
        return $subGroups;
    }
     ...
     * @return  SingularAbstract|Group              provides fluent interface.
     * @throws Exception
     */
    public function setSubgroups($subgroups)
    {
        if (!is_array($subgroups)) {
            throw new \InvalidArgumentException(
                self::FIELD_SUBGROUPS . ' must be specified as an array.'
            );
        }
        ...
        return $this->setRawValue(self::FIELD_SUBGROUPS, $subgroups);
    }
     ...
     * @param $group
     * @return Group|SingularAbstract   provides fluent interface.
     * @throws Exception
     */
    ...
    {
        $owners = $this->getRawValue(self::FIELD_OWNERS) ?: array();
    ...
    public function setOwners($owners)
    {
        if (!is_array($owners)) {
            throw new \InvalidArgumentException(
                self::FIELD_OWNERS . ' must be specified as an array.'
            );
        }
        ...
        foreach ($owners as &$owner) {
            ...
            if (!static::isValidUserId($owner)) {
                throw new \InvalidArgumentException(
                    'Individual ' . self::FIELD_OWNERS . ' must be a valid ID in either string or P4\Spec\User format.'
                );
            }
        }
    ...
    {
        $users = $this->getRawValue(self::FIELD_USERS) ?: array();
        sort($users, SORT_STRING);
        // Investigation of SW-8906 has found that we need to bring back users in lower case to allow other parts of
        // the site function correctly.
        if (!$this->getConnection()->isCaseSensitive()) {
            $users = AbstractKey::lowercase($users);
        }
        return $users;
    }
    ...
    public function setUsers($users)
    {
        if (!is_array($users)) {
            throw new \InvalidArgumentException(
                self::FIELD_USERS . ' must be specified as an array.'
            );
        }
        ...
        foreach ($users as &$user) {
            ...
            if (!static::isValidUserId($user)) {
                throw new \InvalidArgumentException(
                    'Individual ' . self::FIELD_USERS .
                            ' must be a valid ID in either string or P4\Spec\User format.'
                );
            }
        }

        return $this->setRawValue(self::FIELD_USERS, $users);
    }
     ...
     * @return int|mixed|null           (if 'unset'), integer >0 or 'unlimited'
     * @throws Exception
     */
     ...
     * @param   null|int|string         $max    null (or 'unset'), integer >0 or 'unlimited'
     * @return SingularAbstract|Group           provides a fluent interface
     * @throws Exception
     */
    ...
    protected static function getFetchAllFlags($options)
    {
        ...
        if (isset($options[static::FETCH_BY_USER])) {
            ...
            // Fetch by user is along the lines of
            // User -> groups -u <userId>
            // Owner -> groups -o <userId>
            // All(default) -> groups <userId>
            switch ($options[static::FETCH_BY_USER_MODE]??static::USER_MODE_USER) {
