...
class KeyName extends AbstractValidate
{
    ...
    protected $messageTemplates   = array(
        ...
        self::MAX_VALUE                 => "Identifiers too long. Must not be longer than 1024 bytes of UTF-8."
    );
    ...
    public function isValid($value)
    {
        ...
        if (mb_strlen($value) > $this->allowMaxValue) {
            $this->error(static::MAX_VALUE);
            return false;
        }
    ...
    /**
     * Set the max Value allowed for a name.
     *
     * @param int $max The max length of a string.
     */
    public function setAllowMaxValue($max)
    {
        $this->allowMaxValue                     = (int) $max;
        $message                                 = "Identifiers too long. Must not be longer than ".
                                                   $max . " bytes of UTF-8.";
        $this->messageTemplates[self::MAX_VALUE] = $message;
    }


    /**
     * Get the max Value allowed for a name.
     *
     * @return int The value of max Value
     */
    public function getAllowedMaxValue()
    {
