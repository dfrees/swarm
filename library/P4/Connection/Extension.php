...
class Extension extends AbstractConnection
{
    ...
    /**
     * Sets up Saml login details on the P4PHP connection. To be called when the
     * response from the IDP is received to set the necessary fields so that the
     * trigger can validate the response
     * @param string $userId user id
     * @param string $header The saml-agent uses chromium which generates its own stdout messages
     * the work around is that we add a header for the pertinent data for the trigger to look at.
     * @param string $requestField field to examine on _REQUEST when returned from the IDP
     */
    public function setSamlLogin($userId, $header, $requestField)
    {
        $this->instance->loginsso      = true;
        $this->instance->user          = $userId;
        $this->instance->ssopassresult = $header . $_REQUEST[$requestField];
    }
