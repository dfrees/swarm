...
abstract class AbstractConnection implements ConnectionInterface
{
    ...
    const TWOFA                 = '2FA';
    const AUTHEDBY              = 'AuthedBy';
    const TICKETEXPIRATION      = 'TicketExpiration';
    const SPEC_ARCHIVE_FAILED   = "Warning: couldn't archive to spec depot";
    const UNICODE_ONLY_CLIENTS  = 'Unicode server permits only unicode enabled clients.';
    const USE_P4_TRUST          = "To allow connection use the 'p4 trust' command";
    ...
    {
        try {
            $result = $this->run('login', '-s', null, true);
    ...
    public function run(
        ...
        if ($result->hasErrors() && !$ignoreErrors) {
            ...
            //If customer have spec depot enabled sometimes Swarm can be returned a error saying that can't create
            // spec but the group has been created. If we get this we should ignore the error as it isn't Swarm
            // responsibility to handle the spec depot.
            $data = $result->getData();
            if (count($data) > 0 && !is_array($data[0])) {
                preg_match_all("(".preg_quote("Group").".*?".preg_quote("Created").")s", $data[0], $matches);
                if (count($matches) > 0 && (stripos($errors[0], static::SPEC_ARCHIVE_FAILED) !== false)) {
                    return $result;
                }
            }
            ...
            if (!$this->getCharset() && stripos($errors[0], static::UNICODE_ONLY_CLIENTS) !== false) {
            ...
            if (stripos($errors[0], static::USE_P4_TRUST) !== false && !$this->hasTrusted) {
    ...
    public function isSuperUser()
    {
        try {
            ...
            // If the user is not authenticated then they cannot be super
        }
        return isset($maxAccess) && $maxAccess === 'super';
    }
    ...
    public function isAdminUser($allowSuper = false)
    {
        try {
            ...
            // If the user is not authenticated then they cannot be admin
        }
        return isset($maxAccess) && (($allowSuper && $maxAccess === 'super') || $maxAccess === 'admin');
    }
    ...
    /**
     * Finds a value in the array for the key taking into account case sensitivity on the connection
     * @param string    $key        key to find
     * @param mixed     $values     values to search
     * @return mixed the matching value or null if not found
     * @throws P4Exception
     */
    ...
    public function getMaxAccess($host = null)
    {
        ...
        try {
            ...
            if (stristr($errors[0], "empty")) {
                ...
                || stristr($errors[0], "p4 login2")
                || stristr($errors[0], "approval is pending")
            ) {
                // To handle if password must be set or if login2 is required. We deal with the login2
                // in the js
                return false;
            }
    ...
    {
        // call isAuthenticated to force mFAStatus if it has never been set
        if (null === $this->mFAStatus) {
            $this->isAuthenticated();
        }
        return $this->mFAStatus;
    }

    private function setMFAStatus($status)
    {
