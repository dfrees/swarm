...
abstract class FieldedAbstract extends ConnectedAbstract implements FieldedInterface, Serializable
{
    const INDEX          = 'index';
    ...
    /**
     * Implements serialize to only serialize values.
     * @return string
     */
    public function serialize()
    {
        return serialize($this->toArrayStore());
    }
    ...
    {
        $this->setRawValues(unserialize($serialized));
    }
    ...
    public function get($field = null)
    {
        ...
        if (isset($this->fields[$field])
            && is_array($this->fields[$field])
            && isset($this->fields[$field][self::ACCESSOR])
        ) {
            return $this->{$this->fields[$field][self::ACCESSOR]}();
        }
    ...
    {
        // Have to clear the field cache if there is an arbitrary set
        if ($this->fieldsCache && !in_array($field, $this->fieldsCache)) {
        ...
        if (isset($this->fields[$field])
            && is_array($this->fields[$field])
            && isset($this->fields[$field][self::MUTATOR])
        ) {
            return $this->{$this->fields[$field][self::MUTATOR]}($value);
        }
    ...
    public function getFields()
    {
        if (!$this->fieldsCache) {
            ...
            $this->fieldsCache = array_unique(array_merge($this->getDefinedFields(), array_keys($this->values)));
        }
     ...
     * @return  bool                true if the raw value is set
     */
