...
class File extends ConnectedAbstract implements FieldedInterface, ResolvableInterface
{
    ...
    const IMPORT          = 'import';
    const DELETE          = 'delete';
    const IS_ADD_PATTERN  = self::ADD . '|' .self::BRANCH . '|' . self::IMPORT;
    const IS_EDIT_PATTERN = self::IS_ADD_PATTERN . '|' . self::DELETE;
    ...
    const DIRNAME_INVALID_ARGUMENT_ERROR = 'dirname(): Invalid argument, levels must be >= 1';
    const DIRNAME_EXPECTED_PARAMETER_INT = 'dirname() expects parameter 2 to be integer, string given';

    const DIRNAME_EXPECTED_PARAMETER_INT_7_3 = 'dirname() expects parameter 2 to be int, string given';

    protected $cache    = [];
    protected $filespec = null;

    /**
     * Get the file info as an array including all fields.
     * @return  array   the file info as an array.
     */
    public function toArray()
    {
        return $this->toArrayStore();
    }
    ...
    public static function exists($filespec, ConnectionInterface $connection = null, $excludeDeleted = false)
    {
        ...
        if ($result->hasWarnings()) {
            return false;
        } elseif ($excludeDeleted && strstr($result->getData(-1, 'action'), self::DELETE) !== false) {
            return false;
        } else {
    ...
    {
        return $this->openForAction(self::DELETE, $change, null, $force);
    }
    ...
    {
        return $this->openForAction(self::ADD, $change, $fileType, false);
    }
    ...
    public function edit($change = null, $fileType = null, $force = true)
    {
        ...
        return $this->openForAction(self::EDIT, $change, $fileType, $force);
    }
    ...
    public function setAttributes($attributes, $propagate = true, $force = false)
    {
        ...
        try {
            $batches = $connection->batchArgs($params, $prefixParams, [$filespec], 4);
            ...
            foreach ($attributes as $key => $value) {
                ...
                $result = $this->getConnection()->run(
                    'attribute',
                    array_merge($prefixParams, ['-n', $key, $filespec]),
                    bin2hex($value)
                );
    ...
    public function setAttribute($key, $value, $propagate = true, $force = false)
    {
        ...
        return $this->setAttributes([$key => $value], $propagate, $force);
    }
    ...
    public function clearAttributes($attributes, $force = false)
    {
        ...
        $connection   = $this->getConnection();
        $prefixParams = $force ? ['-f'] : [];
        foreach ($connection->batchArgs($params, $prefixParams, [$filespec], 2) as $batch) {
    ...
    {
        return $this->clearAttributes([$attribute], $force);
    }
    ...
    public function getStatus($field = null)
    {
        ...
        if (!array_key_exists('status', $this->cache) || !isset($this->cache['status'])) {
            ...
            $result = $this->getConnection()->run(
                'fstat',
                ['-Oal', $this->getFilespecWithRevision()]
            );
            if ($result->hasWarnings()) {
                throw new Exception(
    ...
    {
        return File::isDeleteAction($this->getStatus('headAction'));
    }
    ...
    {
        return File::isAddAction($this->getStatus('headAction'));
    }
    ...
    public function getDepotContentLines($lines)
    {
        ...
        foreach ($lines as $line) {
            ...
            if (is_string($line)) {
                ...
                $line = ['start' => $matches[1], 'end' => $matches[2]];
            }
            ...
            $ranges[] = ['start' => (int) $line['start'], 'end' => (int) $line['end']];
        }
     ...
     */
    public function getAnnotatedContent(array $options = [])
    {
        // verify we have a filespec set; throws if invalid/missing
        $this->validateHasFilespec();
    ...
    public function getChanges(array $options = null)
    {
        ...
        $options = array_merge(
            (array) $options,
            [Change::FETCH_BY_FILESPEC => $this->getFilespec(true)]
        );
    ...
    protected function openForAction($action, $change = null, $fileType = null, $force = true)
    {
        ...
        if (!in_array($action, [self::ADD, self::EDIT, self::DELETE])) {
        ...
        if ($force
            ...
            && !($action == self::EDIT && $this->isOpenForAction('integrate'))
            && !($action == self::ADD  && $this->isOpenForAction('branch'))
        ) {
            ...
            if ($result->getData(0, 'oldAction') === self::DELETE
                && $result->getData(0, 'action') === 'cleared'
                && $result->getData(0, 'haveRev') !== 'none'
            ) {
                $p4->run(
                    'sync',
                    ['-f', $this->getFilespecWithRevision()]
                );
            }
        }
        ...
        $result = $p4->run($action, array_merge(['-n'], $flags));
        foreach ($result->getData() as $data) {
            if (is_string($data)
    ...
    protected function isOpenForAction($action)
    {
        ...
        if ($openAction == 'move/add' && $action == self::EDIT) {
            return true;
        }
    ...
    /**
     * Get the common path for an array of paths
     *
     * @param array $paths          The paths we want to find the base.
     * @param bool  $caseSensitive  Is the server in case sensitive.
     *
     * @return bool|string
     */
    public static function getCommonPath(array $paths, $caseSensitive = false)
    {
        if (empty($paths) || is_null($paths)) {
            return '';
        }
        // If the server is case sensitive leave the paths otherwise lowercase all.
        $paths      = $caseSensitive ? $paths : array_map('strtolower', $paths);
        $lastOffset = 1;
        $common     = '/';
        // First look though the paths and remove any that don't contain depot paths
        // this would normally be done earlier but in case a invalid path gets though.
        foreach ($paths as $key => $path) {
            // As we are only getting common path we don't care about the exclusion or overlay options
            $paths[$key] = $path = strpos($path, '-') === 0 || strpos($path, '+') === 0 ? substr($path, 1): $path;
            // If path isn't a depot path remove it from list.
            if (strpos($path, '//') !== 0) {
                unset($paths[$key]);
            }
        }
        while (($index = strpos($paths[0], '/', $lastOffset)) !== false) {
            $dirLen = $index - $lastOffset + 1; // include the forward slash
    ...
    /**
     * Due to a issue with PHP 5.3 can't take a depth level we have to loop though the path till we get to
     * the level we want to get to.
     *
     * @param string $path   This is the Path that we want to shorten
     * @param int    $level  The number of parent directories to go up. This must be an integer greater than 0.
     *
     * @return string
     */
    public static function dirname($path, $level = 1)
    {
        if (is_string($level) && !is_numeric($level)) {
            if (version_compare(PHP_VERSION, '7.3', '<')) {
                return self::DIRNAME_EXPECTED_PARAMETER_INT;
            } else {
                return self::DIRNAME_EXPECTED_PARAMETER_INT_7_3;
            }
        } elseif (is_numeric($level) && (int) $level < 1) {
            return self::DIRNAME_INVALID_ARGUMENT_ERROR;
        } elseif (is_bool($level) && $level == false) {
            return self::DIRNAME_INVALID_ARGUMENT_ERROR;
        }
    ...
    /**
     * Determine if the file action represents an add. 'add', 'branch' and 'import' are add actions.
     * @param mixed $fileAction
     * @return bool
     */
    public static function isAddAction($fileAction)
    {
        return preg_match('/' . self::IS_ADD_PATTERN. '/', $fileAction) !== 0;
    }

    /**
     * Determine if the file action represents an edit. An action that is NOT 'add', 'branch', 'import' or
     * 'delete' are considered edit. (For example 'edit' and 'integrate'
     * @param mixed $fileAction
     * @return bool
     */
    public static function isEditAction($fileAction)
    {
        return preg_match('/' . self::IS_EDIT_PATTERN . '/', $fileAction) === 0;
    }

    /**
     * Determine if the file action represents an delete. 'delete' is the only delete action.
     * @param mixed $fileAction
     * @return bool
     */
    public static function isDeleteAction($fileAction)
    {
        return strpos($fileAction, self::DELETE) !== false;
    }
}
