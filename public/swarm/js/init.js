...
$(function() {
    ...
    $(document).on('swarm-login', function(e) {
        ...
        if (e.user !== undefined) {
            ...
            $('.session-menu .user').replaceWith($(e.toolbar).find('.user'));
            // reveal if mfa is required.
            if (e.user.mfa) {
                $('body').removeClass('mfa-required');
                if(e.user.mfa === "required") {
                    $('body').addClass('mfa-required');
                    return;
                }
            }
        }

        // If user has logged in and we are on dashboard start it.
        if ($('#dashboard-tab').length > 0) {
            swarm.dashboard.init();
        }
        // Setup the time format for user.
        swarm.timeFormatter.init();
    });
    ...
    // For the review Preview switch we use cookie. If the user cookie is not set we will create a cookie for review_ui.
    // If the config is set we will use that value otherwise we will default to classic.
    if (!document.cookie.split('; ').find(function(row) { return row.startsWith('review_ui');})) {
        swarm.setCookie(
            "review_ui",
            swarm.config.globalConfig['reviews.default_ui']||'classic',
            (new Date('January 1, 2025')).toUTCString());
    }
});
...
$(document).on('ajaxError', function(event, request, settings, message) {
    ...
    if (!$('body').hasClass('mfa')) {
        ...
        var notification = $('<div />', {'id': 'message', 'class': 'global-notification border-box'})
            .append(alertHTML).prependTo('#swarm-body-container');
